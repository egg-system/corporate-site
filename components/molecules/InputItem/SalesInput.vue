<template>
  <div>
    <div class="flex center columns">
      <input-label class="column is-4" label="年間売上高" />
      <input-box
        :text="text"
        class="column"
        label="例）10,000,000,000"
        @focus="focus"
        @blur="blur"
      />円
    </div>
...
export default {
  ...
  computed: {
    requiredErrorFlg: function() {
      return this.$store.state.simulator.requiredErrorFlg.sales
    },
    validationErrorFlg: function() {
      return this.$store.state.simulator.validationErrorFlg.sales
    }
  },
  methods: {
    blur(value) {
      ...
      this.$store.dispatch('simulator/commitSales', Number(value))
    },
    focus(value) {
    ...
    validation(value) {
      ...
      this.$store.dispatch('simulator/commitSalesValidationError', errorFlg)
      return errorFlg
    },
    required(value) {
      var errorFlg = value == ''
      this.$store.dispatch('simulator/commitSalesRequiredError', errorFlg)
    }
  }
}
</script>
